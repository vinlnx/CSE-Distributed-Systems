...
public class Emulator extends Manager {
    ...
    protected void start() {
        ...
        synPartialOrderLogger
                .start(MessageLayer.synopticPartialOrderLogFilename);
        ...
        if (cmdInputType == InputType.FILE) {
            while (node != null || failed) {
                if (IOFinished && node != null) {
                    System.err
                            .println("Network I/O thread failed, killing the node...");
            ...
            while (node != null || failed) {
                ...
                if (node == null) {
                    ...
                    /*
                     * do{ // just in case an exception is thrown or input is
                     * null Event ev = null;
                     * 
                     * try{ // A command will be converted into an Event and
                     * passed to the node later in the loop // A quit command
                     * will be matched later in this try block //
                     * Empty/whitespace will be treated as a skipped line, which
                     * will return null and cause a continue String input =
                     * keyboard.readLine(); // Process user input if there is
                     * any if (input != null) { ev = parser.parseLine(input); }
                     * } catch (IOException e) {
                     * System.err.println("Error on user input: " + e); }
                     * 
                     * if (ev == null) { advance = true; } else { if (ev.t ==
                     * Event.EventType.TIME) { advance = true; } else {
                     * currentRoundEvents.add(ev); } } } while (!advance);
                     */
                } else {
                    ArrayList<Event> currentRoundEvents = new ArrayList<Event>();
    ...
    private void startNode() {
        ...
        if (Replay.isReplaying()) {
            ...
            try {
                ...
                if (Replay.isAddrPacket(addrPkt)) {
                    ...
                    throw new Replay.ReplayException(
                            "Address packet expected! Instead, we got: "
                                    + addrPkt.toString());
                }
            } catch (CorruptPacketException e) {
                throw new Replay.ReplayException(
                        "Address packet expected, but packet was corrupted");
            }
        } else {
        ...
        if (address == Manager.BROADCAST_ADDRESS) {
            // Router returns broadcast address to signal it has no more free
            // addresses
            System.err
                    .println("We couldn't get a port from the Router.  Maybe there are no more free addresses?");
            stop();
        }
    ...
    protected void sendPkt(Node fromNode, int to, int protocol, byte[] payload)
        ...
        logEvent(fromNode, "SEND " + newPacket.toSynopticString(fromNode)); // XXX:
                                                                            // broadcasts
                                                                            // are
                                                                            // one
                                                                            // msg
